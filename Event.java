 ...
 * class Event takes date and eventName as a parameter
 * all variables are validated by their own standards
 * has overloading constructor to take event as a String
 * Has method toString to display the eventName and date as a String
 * Has method to compare two events to compare which is bigger, newer dates are bigger
 * Subclass of Dates which has an ArrayList of Events
 * has a subclass enum which holds all event names
 */
